...
UpdateTab::UpdateTab() {
    ...
    this->label = new brls::Label(brls::LabelStyle::DESCRIPTION, fmt::format("{}\n {} {}", "menu/label/desc_smd"_i18n, "menu/label/new_update"_i18n, APP_VER), true);
    this->addView(label);

    this->item = new brls::ListItem("menu/item/check_update"_i18n);
    item->setHeight(50);
    item->getClickEvent()->subscribe([this](auto* view) {
        ...
        if(this->json.size()) {
            ...
            this->label = new brls::Label(brls::LabelStyle::DESCRIPTION, fmt::format("{} {}", "menu/label/available_version"_i18n, this->version), true);
            this->addView(label);
